...
import com.kutuphane.akillikutuphane.Service.MailService;
...
public class OduncAlmaSistemiService {
    ...
    public OduncAlmaSistemiService(OduncAlmaSistemiRepository oduncRepository, KitapRepository kitapRepository,
            CezaSistemiRepository cezaRepository, MailService mailService) {
    ...
    public void kitapIade(Integer oduncId) {
        ...
        oduncRepository.saveAndFlush(odunc);

        Object result = oduncRepository.findRawMailData(oduncId);

        if (result != null) {
            Object[] row = (Object[]) result;
            String email = (String) row[0];
            String kitapAd = (String) row[1];

            Integer ceza = cezaRepository.findByOduncId(oduncId)
                    .map(CezaSistemi::getCezaMiktari)
                    .orElse(0);

            try {
                mailService.iadeMailiGonder(email, kitapAd, ceza);
            } catch (Exception e) {
    ...
    // İADE EDİLENLER
    public List<OduncAlmaSistemi> kullanicininİadeleri(int kullaniciId) {
        return oduncRepository.iadeEdilenler(kullaniciId);
    }
    ...
    // TÜM AKTİF ÖDÜNÇLER
    public List<OduncAlmaSistemi> tumAktifOduncler() {
        return oduncRepository.findByGercekIadeTarihiIsNull();
    }

    // TÜM İADELER
    public List<OduncAlmaSistemi> tumIadeler() {
        return oduncRepository.findByGercekIadeTarihiIsNotNull();
    }
}
