...
window.adminShowKitaplar = async function () {
    ...
    kitaplar.forEach(k => {
        ...
        actions.innerHTML = `
            <button onclick="kitapGuncelle(${k.kitapId})">✏️</button>
...
window.adminShowKullanicilar = async function () {
    ...
    users.forEach(u => {
        ...
        r.insertCell().innerText = u.cezaMiktarı;
...
window.kitapGuncelle = async function (id) {
    const ad = prompt("Yeni ad:");
    const stok = prompt("Yeni stok:");
    const yazarId = prompt("Yazar ID:");

    await apiFetch(`/kitap/${id}?yazarId=${yazarId}`, {
        method: "PUT",
        body: JSON.stringify({ kitapAd: ad, stokAdedi: stok })
    });
    window.adminShowKitaplar();
