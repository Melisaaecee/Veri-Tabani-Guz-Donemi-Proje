...
function hideAllSections() {
    ["kitapListesi", "yazarListesi", "kategoriListesi", "oduncListesi", "cezaListesi"]
...
window.showKitaplar = async function () {
    ...
    kitaplar.forEach(k => {
        ...
        btn.onclick = () => {
            const tarih = prompt("İade tarihi (YYYY-MM-DD)");
            if (tarih) oduncAl(k.kitapId, tarih);
        };
...
window.showKategoriler = async function () {
    ...
    const table = document.createElement('table');
    table.innerHTML = `<tr><th>ID</th><th>Ad</th></tr>`;
...
window.showCezalar = async function () {
    ...
    const table = document.createElement('table');
    table.innerHTML = `<tr><th>Kitap</th><th>Ceza</th></tr>`;
...
async function kitapIade(oduncId) {
    await apiFetch(`/odunc/iade/${oduncId}`, {
        method: "PUT"
    });
